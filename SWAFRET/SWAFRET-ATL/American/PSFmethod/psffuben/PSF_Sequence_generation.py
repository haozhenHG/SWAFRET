...
from Australia.Readcsv import read_48_columns_featurecsv
...
def reshape24_1(usedata):
    np.array(usedata.values.flatten()).reshape(24, 1)
...
def PSF_Sequence(Ydat, traindata, label, w, testday):
    ...
    # 筛选ES_d：选择mape误差小的   返回更加精确筛选的序列
    # 这种筛选可能出现的  1. 原序列  < 50  无需筛选
    #                  2. 原序列 >50 筛选后的长度    a. >50  继续对这部分数据进行筛选
    #                                             b. <50 无需筛选
    ...
    if len(ES_d) > 0:
        ...
        if len(yuan):  # 强相关数据存在
            ...
            if len(weekday) != 0 and len(season) != 0:
                y_pred = (sum(sum_weekday) / len(weekday)) * 0.4 + (sum(sum_season) / len(season)) * 0.6
            elif len(weekday) != 0 and len(season) == 0:
                y_pred = sum(sum_weekday) / len(weekday)
            elif len(weekday) == 0 and len(season) != 0:
                y_pred = sum(sum_season) / len(season)
