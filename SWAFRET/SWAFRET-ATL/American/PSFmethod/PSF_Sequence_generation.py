...
import numpy as np
import pandas as pd
def read_27clomuns_feature():
    wsname = '..\\CSVData\\27clomuns_feature.csv'
...
def Data_filtering(ES_d_COPY,data, true_values, num=2):
    ...
    if num == 2:
        ...
        for date in ES_d_COPY:
            predicted_values = data.loc[date]['load1':'load24'].values
...
def PSF_Sequence(Ydat, traindata, label, w, testday):
    ...
    :return:
    '''
    traindata = traindata.loc['2019-01-01':'2020-12-31']
    if w < 1:
        return 0
    ...
    ES_d_COPY = ES_d
    data = read_27clomuns_feature()
    true_values = data.loc[testday]['load1':'load24'].values  # 真实值    # 根据一天的长度  要改 这是48

    if len(ES_d) >= 5:
        ES_d = []
        for date in ES_d_COPY:
            predicted_values = data.loc[date]['load1':'load24'].values  # 根据一天的长度  要改 这是48
            mape = Average_Relative_Error(true_values, predicted_values)
