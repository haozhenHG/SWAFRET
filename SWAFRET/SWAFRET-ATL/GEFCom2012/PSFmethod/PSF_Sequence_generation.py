...
import numpy as np
import pandas as pd
def read_data_loadcsv():
    wsname = '..\\CSVData\\data_load.csv'
...
def Data_filtering(ES_d_COPY,data, true_values, num=2):
    ...
    if num == 2:
        ...
        for date in ES_d_COPY:
            predicted_values = data.loc[date]['1h':'24h'].values
...
def PSF_Sequence(Ydat, data_real,traindata ,label, w, testday):
    '''
    ...
    :return:
    '''
    traindata = traindata.loc['2004-01-01':'2007-10-30']
    if w < 1:
        return 0
    ...
    ES_d_COPY = ES_d
    data = read_data_loadcsv()
    true_values = data.loc[testday]['1h':'24h'].values  # 真实值    # 根据一天的长度  要改 这是48

    if len(ES_d) >= 5:
        ES_d = []
        for date in ES_d_COPY:
            predicted_values = data.loc[date]['1h':'24h'].values  # 根据一天的长度  要改 这是48
            mape = Average_Relative_Error(true_values, predicted_values)
            print('compare - mape : ',mape)
            if mape < 3: # 6->8
                ES_d.append(date)
            elif w == 1 and mape < 4.5:
                ES_d.append(date)
            elif w == 1 and mape < 5:
                ES_d.append(date)
            elif w == 1 and mape < 6:
    ...
    if len(ES_d) > 0:
        ...
        sum_yuan = traindata.loc[yuan, 'load'].values.reshape(-1, 24)  # shape :(len(yuan),24)
        sum_weekday = traindata.loc[weekday, 'load'].values.reshape(-1, 24)
        sum_season = traindata.loc[season, 'load'].values.reshape(-1, 24)
        sum_no_fea = traindata.loc[no_fea, 'load'].values.reshape(-1, 24)
        ...
        if len(yuan):  # 强相关数据存在
            ...
            if len(weekday) != 0 and len(season) != 0:
                y_pred = (sum(sum_weekday) / len(weekday)) * 0.24 + (sum(sum_season) / len(season)) * 0.76
            elif len(weekday) != 0 and len(season) == 0:
                y_pred = sum(sum_weekday) / len(weekday) * 0.95
            elif len(weekday) == 0 and len(season) != 0:
                y_pred = sum(sum_season) / len(season)   * 0.95
            elif len(sum_no_fea) !=0:
                y_pred = (sum(sum_no_fea) / len(no_fea)) * 0.9
            else:
                w = w - 1
                if w < 1:
                    ...
                    y_pred = PSF_Sequence(Ydat, data_real,traindata, label, w, testday=testday)
    else:
        w = w - 1
        if w < 1:
            raise ValueError("参数w必须大于或等于1")
